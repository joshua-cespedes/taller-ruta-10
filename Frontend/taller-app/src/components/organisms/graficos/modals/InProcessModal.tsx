...
export function InProcessModal(props: {
  isOpen: boolean;
  ...
  return (
    ...
    >
      {props.itemsTotal.length === 0 ? (
        <div style={styles.empty}>No hay citas en proceso.</div>
      ) : (
          ...
          {props.pageItems.map((a) => (
            <div key={a.id} style={styles.modalRow}>
              ...
              <div style={styles.modalRowRight}>
                ...
                <span style={{ ...styles.tag, ...styles.tagInfo }}>En Proceso</span>
              </div>
            </div>
          ))}
        </div>
